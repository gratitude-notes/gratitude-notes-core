...
const WriteNoteForm: React.FC<FormHandlerProps> = ({ updateViewState }) => {
    ...
    const checkTimePeriod = (timestampToCheck: Timestamp): number => {
        ...
        if (
            ...
            timestampToCheck.isEqual(Timestamp.fromMillis(0))
        ) {
  ...
  const getEmojiScore = (score: number) => {
    ...
    const fetchLocationName = async (latitude: number, longitude: number) => {
        const repsonse = await fetch(
            `https://maps.googleapis.com/maps/api/geocode/json?latlng=${latitude},${longitude}&result_type=locality&key=${
                import.meta.env.VITE_GCP_MAPS_API_KEY
            }`
        );
    ...
    const generateKeywords = async (text: string) => {
        ...
        const nlpResponse = await fetch(
            `https://language.googleapis.com/v1beta2/documents:annotateText?key=${
                import.meta.env.VITE_GCP_NLP_API_KEY
            }`,
            {
                method: 'POST',
                headers: {
        ...
        if (nlpResponse.ok) {
            ...
            const entities = nlpDataEntitiesArray.map(({ name, salience }: { name: string; salience: number }) => ({
                name,
                salience,
            }));

            entities.sort(
                (a: { name: string; salience: number }, b: { name: string; salience: number }) =>
            ...
            <div className="px-2 flex-grow bg-white dark:bg-gray-800">
                <LexicalComposer {...{ initialConfig }}>
                    ...
                    <div className="relative">
                        <RichTextPlugin
                            contentEditable={
                                <ContentEditable className="min-h-[100px] max-h-[200px] overflow-y-scroll outline-none scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200 dark:scrollbar-track-gray-700" />
                            }
                            placeholder={
