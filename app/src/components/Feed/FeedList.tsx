...
const FeedList: React.FC = () => {
    ...
    const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        const results = bullets?.filter((bullet: NoteBullet) => {
            ...
            switch (searchCategory.toLowerCase()) {
                ...
                case 'day':
                    return bulletDay?.includes(searchQuery);
                case 'month':
                    return bulletMonth?.includes(searchQuery);
                case 'date':
                    return bulletDate?.includes(searchQuery);
                case 'year':
                    return bulletYear?.includes(searchQuery);
                case 'score':
                    return bulletScore === searchQuery;
                case 'location':
                    return bulletAddress?.includes(searchQuery);

                // General searching (All)
                default: {
                    return (
                        bulletTextContent.includes(searchQuery) ||
    ...
    const handleSearchHowTo = () => {
        searchGuideVisible.isComponentVisible
            ? searchGuideVisible.setComponentVisible(false)
            : searchGuideVisible.setComponentVisible(true);
        handleTopOfList();
    };
    ...
    return (
        ...
        >
            <div className="sticky -top-[1px] pb-1 bg-white dark:bg-gray-800 z-40 border-b border-black dark:border-gray-400">
                {/* FEED SELECTOR */}
                <FeedSelector
                    handleTopOfList={handleTopOfList}
                    feedSelection={feedSelection}
                    handleFeedSelectorChange={handleFeedSelectorChange}
                />
