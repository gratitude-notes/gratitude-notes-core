...
const SlidingSequence: React.FC<SlidingSequenceProps> = ({ colorHeart, slidingDirection, position }) => {
    ...
    const contentPosition = interpolate(progress, [0, 1], [-contentWidth, 0]);
    ...
    return (
        <div className={`fixed ${position === 'Bottom' ? 'top-[92%]' : 'top-0'}`} style={{ width, height }}>
            {trail}
            <div
                style={{
                    position: 'absolute',
...
const Heart: React.FC<{ colorHeart: string }> = ({ colorHeart }) => {
    return <AbsoluteFill>{colorHeart === 'Red' ? '❤️' : '💛'}</AbsoluteFill>;
};
